...
internal open class BufferedChannel<E>(
    ...
    private val expandBufferCompleted = atomic(bufferEnd.value)
    ...
    init {
        ...
        // In case of cancellation, the algorithm requires access to the send and receive operation counters,
        // as well as to the `onUndeliveredElement()` lambda.
        // The latter can also be solved by passing the lambda to the cancellation handler when suspending.
    ...
    private fun CancellableContinuation<*>.prepareSenderForSuspension(
        ...
        if (onUndeliveredElement == null) {
            ...
            // TODO: mechanism  for segments when #???? is finished.
    ...
    override fun trySend(element: E): ChannelResult<Unit> {
        ...
        // the only difference that we store `INTERRUPTED_SEND` in case
    ...
    private inline fun <R> sendImpl(
        ...
        while (true) {
            ...
            if (segment.id != id) {
                if (segment.id > id) {
                    sendersAndCloseStatus.compareAndSet(sendersAndCloseStatusCur,
                        constructSendersAndCloseStatus(segment.id * SEGMENT_SIZE, sendersAndCloseStatusCur.closeStatus))
            ...
            when(updateCellSend(segment, i, element, s, if (closed) INTERRUPTED_SEND else waiter)) {
                ...
                RESULT_SUSPEND -> {
                    ...
                    // and the `INTERRUPTED_SEND` token has been stored as a waiter,
     ...
     * TODO: fast-path slow-path technique
     */
    private fun updateCellSend(
        ...
        while (true) {
            ...
            when {
                ...
                state === POISONED -> return RESULT_FAILED
    ...
    private suspend fun receiveOnNoWaiterSuspend(
        ...
        receiveImplOnNoWaiter( // <-- this is an inline function
            ...
            onSuspend = { segm, i -> cont.prepareReceiverForSuspension(segm, i) },
    ...
    */
    ...
    protected fun tryReceiveInternal(): ChannelResult<E> {
        ...
        return receiveImpl( // <-- this is an inline function
            ...
            // TODO: `segm.onSlotCleaned()` should be called here.
            // TODO: The current implementation relies on the invariant
            // TODO: that physically removed segments were full of
            // TODO: cancelled senders. Obviously, this invariant causes
            // TODO: memory leaks, preventing removing segments with
            // TODO: cancelled receivers.
            onSuspend = { segm, _ ->
                waitExpandBufferCompletion(r)
                segm.onSlotCleaned()
                failure()
            },
            // If the channel is closed, return the corresponding result.
    ...
    private inline fun <R> receiveImpl(
        ...
        @Suppress("NAME_SHADOWING") var waiter = waiter
        var cancelled = false
        while (true) {
            ...
            val r = this.receiversAndCancelStatus.getAndIncrement().let {
                if (it.closeStatus == CLOSE_STATUS_CANCELLED) {
                    waiter = INTERRUPTED_RCV
                    cancelled = true
                }
                it.counter
    ...
    private inline fun <W, R> receiveImplOnNoWaiter(
        ...
        by making a rendezvous with a suspended sender. */
        onElementRetrieved: (element: E) -> R,
        /* This lambda is called when the operation suspends in
        the cell specified by the segment and the index in it. */
        onSuspend: (segm: ChannelSegment<E>, i: Int) -> R,
        /* This lambda is called when the channel is observed
    ...
    private fun updateCellReceive(
        ...
        while (true) {
            ...
            when {
                ...
                state === BUFFERED -> if (segment.casState(i, state, DONE_RCV)) {
    ...
    private fun expandBuffer() {
        ...
        try_again@ while (true) {
            ...
            if (s <= b) {
                if (segment.id < id) {
                    while (true) {
                        // TODO what if a receiver stored into IN_BUFFER? How to deal with that? No fucking idea...
                        val ss = findSegmentBufferOrLast(id, segment)
                        if (bufferEndSegment.moveForward(ss)) {
                            expandBufferCompleted.incrementAndGet().also {
                                if (it.closeStatus != 0) {
                                    while (expandBufferCompleted.value.closeStatus != 0) {}
                                }
                            }
                            return
                        } else {
...
//                bufferEnd.compareAndSet(b + 1, segment.id * SEGMENT_SIZE)
    ...
    internal fun waitExpandBufferCompletion(globalIndex: Long) {
        ...
        repeat(EXPAND_BUFFER_COMPLETION_WAIT_ITERATIONS) {
            ...
            val ebCompleted = expandBufferCompleted.value.counter
            if (b == ebCompleted && b == bufferEnd.value) return
        }
        expandBufferCompleted.update { constructSendersAndCloseStatus(it.counter, 1) }
        while (true) {
            val b = bufferEnd.value
            val ebCompletedAndBit = expandBufferCompleted.value
            val ebCompleted = ebCompletedAndBit.counter
            if (b == ebCompleted && b == bufferEnd.value) {
                expandBufferCompleted.update { constructSendersAndCloseStatus(it.counter, 0) }
                return
            }
            if (ebCompletedAndBit.closeStatus == 0) {
                expandBufferCompleted.compareAndSet(ebCompletedAndBit, constructSendersAndCloseStatus(ebCompleted, 1))
            }
        }
    }
...
//                                segment.onSlotCleaned()
    ...
    protected open fun registerSelectForReceive(select: SelectInstance<*>, ignoredParam: Any?) =
        receiveImpl( // <-- this is an inline function
            ...
            onSuspend = { segm, i -> select.prepareReceiverForSuspension(segm, i) },
...
//        if (segm.id > rcvSegment.id) error("WTF is going on??!!")
        ...
        while (true) {
            ...
            if (segm.id != id) {
                ...
                segm = findSegmentHasElements(id, segm).let {
                    ...
                    if (!rendezvousOrUnlimited && it.segment.id <= bufferEndCounter / SEGMENT_SIZE) {
                        xxx@while (true) {
                            if (bufferEndSegment.moveForward(it.segment)) {
                                break@xxx
                            } else {
                                if (it.segment.isTail) {
                                    bufferEndSegment.value = it.segment
                                    break@xxx
                                }
                            }
                        }
    ...
    override fun toString(): String {
        ...
        sb.append("S=${sendersAndCloseStatus.value.counter},R=${receiversAndCancelStatus.value.counter},B=${bufferEnd.value},C=${sendersAndCloseStatus.value.closeStatus},")
...
internal class ChannelSegment<E>(id: Long, prev: ChannelSegment<E>?, pointers: Int) : Segment<ChannelSegment<E>>(id, prev, pointers) {
    ...
    private fun onCancellationImpl(index: Int): Any? {
        ...
        // TODO: need (must!) overcome this restriction.
